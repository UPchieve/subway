...
describe('getPostSessionSurveyResponse', () => {
  test('Returns postsession survey response', async () => {
    ...
    mockedSurveyRepo.getPostsessionSurveyResponse.mockResolvedValue([
      {
        userRole: 'volunteer',
        questionText:
          "%s's goal for this session was to %s. Were you able to help them achieve their goal?",
        replacementColumnOne: 'student_name',
        replacementColumnTwo: 'student_goal',
        response: 'Somewhat',
        score: 3,
        displayOrder: 10,
      },
      {
        ...
        replacementColumnOne: 'student_name',
        replacementColumnTwo: 'subject_name',
        response: 'Optimistic',
        score: 4,
        displayOrder: 30,
      },
      {
        ...
        replacementColumnOne: undefined,
        replacementColumnTwo: undefined,
        response: 'No',
        score: 0,
        displayOrder: 40,
      },
      {
        ...
        replacementColumnOne: undefined,
        replacementColumnTwo: undefined,
        response: "volunteer's thoughts",
        score: 0,
        displayOrder: 70,
      },
    ])

    const actualSurveyResponse =
      await SurveyService.getPostsessionSurveyResponse('session-id', 'student')

    expect(actualSurveyResponse).toBeDefined()
    expect(actualSurveyResponse[0].userRole).toBe('volunteer')
    expect(actualSurveyResponse[0].displayLabel).toBe(
      `Student's goal for this session was to eat cake. Were you able to help them achieve their goal?`
    )
    expect(actualSurveyResponse[0].response).toBe('Somewhat')
    expect(actualSurveyResponse[1].displayLabel).toBe(
      `How do you think Student feels about Prealgebra at the end of this session?`
    )
    expect(actualSurveyResponse[1].response).toBe('Optimistic')
  })
})
