...
const getVolunteersForWaitingReferencesIR: any = {"usedParamSet":{"start":true,"end":true},"params":[{"name":"start","required":true,"transform":{"type":"scalar"},"locs":[{"a":811,"b":817}]},{"name":"end","required":true,"transform":{"type":"scalar"},"locs":[{"a":858,"b":862}]}],"statement":"SELECT\n    users.id,\n    users.first_name,\n    users.last_name,\n    users.phone,\n    users.email,\n    volunteer_partner_orgs.key AS volunteer_partner_org\nFROM\n    users\n    JOIN volunteer_profiles ON volunteer_profiles.user_id = users.id\n    LEFT JOIN volunteer_partner_orgs ON volunteer_partner_orgs.id = volunteer_profiles.volunteer_partner_org_id\n    LEFT JOIN volunteer_references ON volunteer_references.user_id = users.id\n    LEFT JOIN volunteer_reference_statuses ON volunteer_reference_statuses.id = volunteer_references.status_id\nWHERE\n    users.banned IS FALSE\n    AND users.ban_type IS DISTINCT FROM 'complete'\n    AND users.deactivated IS FALSE\n    AND users.deleted IS FALSE\n    AND users.test_user IS FALSE\n    AND volunteer_reference_statuses.name = 'sent'\n    AND volunteer_references.sent_at > :start!\n    AND volunteer_references.sent_at < :end!\nGROUP BY\n    users.id,\n    volunteer_partner_orgs.key"};
...
/** 'GetVolunteersToBackfillBgInfoUserAction' parameters type */
export type IGetVolunteersToBackfillBgInfoUserActionParams = void;

/** 'GetVolunteersToBackfillBgInfoUserAction' return type */
export interface IGetVolunteersToBackfillBgInfoUserActionResult {
  id: string;
  occupationFirstSetAt: Date | null;
}

/** 'GetVolunteersToBackfillBgInfoUserAction' query type */
export interface IGetVolunteersToBackfillBgInfoUserActionQuery {
  params: IGetVolunteersToBackfillBgInfoUserActionParams;
  result: IGetVolunteersToBackfillBgInfoUserActionResult;
}

const getVolunteersToBackfillBgInfoUserActionIR: any = {"usedParamSet":{},"params":[],"statement":"WITH volunteers_with_occupation AS (\n    SELECT\n        u.id,\n        vo.created_at AS occupation_set_at\n    FROM\n        users u\n        JOIN volunteer_occupations vo ON vo.user_id = u.id\n),\nvolunteers_with_earliest_occupation AS (\n    SELECT\n        id,\n        min(occupation_set_at) AS occupation_first_set_at\nFROM\n    volunteers_with_occupation\nGROUP BY\n    id\n),\nvolunteers_with_no_bg_info_user_action AS (\n    SELECT\n        u.id,\n        u.occupation_first_set_at\n    FROM\n        volunteers_with_earliest_occupation u\n        JOIN volunteer_profiles vp ON vp.user_id = u.id\n        LEFT JOIN user_actions ua ON ua.user_id = u.id\n            AND ua.action = 'COMPLETED BACKGROUND INFORMATION'\n    WHERE\n        ua.id IS NULL\n        AND u.occupation_first_set_at >= '2026-04-28'\n        AND u.occupation_first_set_at IS NOT NULL\n)\nSELECT DISTINCT ON (id)\n    u.id,\n    u.occupation_first_set_at\nFROM\n    volunteers_with_no_bg_info_user_action u"};
 ...
 * ```
 */
export const getVolunteersToBackfillBgInfoUserAction = new PreparedQuery<IGetVolunteersToBackfillBgInfoUserActionParams,IGetVolunteersToBackfillBgInfoUserActionResult>(getVolunteersToBackfillBgInfoUserActionIR);


/** 'BackfillBgInfoUserActionForVolunteers' parameters type */
export type IBackfillBgInfoUserActionForVolunteersParams = void;

/** 'BackfillBgInfoUserActionForVolunteers' return type */
export interface IBackfillBgInfoUserActionForVolunteersResult {
  action: string | null;
...
/** 'BackfillBgInfoUserActionForVolunteers' query type */
export interface IBackfillBgInfoUserActionForVolunteersQuery {
  params: IBackfillBgInfoUserActionForVolunteersParams;
  result: IBackfillBgInfoUserActionForVolunteersResult;
}

const backfillBgInfoUserActionForVolunteersIR: any = {"usedParamSet":{},"params":[],"statement":"WITH volunteers_with_occupation AS (\n    SELECT\n        u.id,\n        vo.created_at AS occupation_set_at\n    FROM\n        users u\n        JOIN volunteer_occupations vo ON vo.user_id = u.id\n),\nvolunteers_with_earliest_occupation AS (\n    SELECT\n        id,\n        min(occupation_set_at) AS occupation_first_set_at\nFROM\n    volunteers_with_occupation\nGROUP BY\n    id\n),\nvolunteers_with_no_bg_info_user_action AS (\n    SELECT\n        u.id,\n        u.occupation_first_set_at\n    FROM\n        volunteers_with_earliest_occupation u\n        JOIN volunteer_profiles vp ON vp.user_id = u.id\n        LEFT JOIN user_actions ua ON ua.user_id = u.id\n            AND ua.action = 'COMPLETED BACKGROUND INFORMATION'\n    WHERE\n        ua.id IS NULL\n        AND u.occupation_first_set_at >= '2026-04-28'\n        AND u.occupation_first_set_at IS NOT NULL)\n    INSERT INTO user_actions (user_id, action, created_at)\n    SELECT\n        u.id,\n        'COMPLETED BACKGROUND INFORMATION',\n        u.occupation_first_set_at\n    FROM\n        volunteers_with_no_bg_info_user_action u\n    RETURNING\n        *"};
 ...
 * ```
 */
export const backfillBgInfoUserActionForVolunteers = new PreparedQuery<IBackfillBgInfoUserActionForVolunteersParams,IBackfillBgInfoUserActionForVolunteersResult>(backfillBgInfoUserActionForVolunteersIR);
