...
export async function getLegacyUserObject(
  ...
  try {
    ...
    const legacyCertificationsResult =
      await pgQueries.getLegacyCertifications.run(undefined, getClient())
    ...
    if (roleContext.isActiveRole('volunteer')) {
      ...
      volunteerUser.certifications = {
        ...
        ...(await getQuizzesForVolunteers([userId]))[userId],
        ...(await getCertificationsForVolunteer([userId]))[userId],
      }
      const totalActiveCerts = Object.keys(
        (await getActiveQuizzesForVolunteers([userId]))[userId]
      ).length
