  ...
  async.waterfall(
    [
      function(callback) {
        ...
        User.find({
          $and: [
            { phone: { $regex: US_PHONE_REGEX } },
            { phone: { $not: STRICT_PHONE_REGEX } }
          ]
        })
          .select('email phone')
          .comment(
            'Search for valid U. S. phone numbers not correctly formatted for twilio'
          )
          .exec(function(err, users) {
            if (err) {
              ...
              users.forEach(function(user) {
                ...
                user.save(function(err) {
                  if (err) {
                    ...
                    console.log(
                      chalk.cyan(
                        `Phone number of user ${user.email} reformatted to ${user.phone} (was ${oldPhone}).`
                      )
                    )
              ...
              console.log(
                'All users with valid phone numbers are formatted correctly.'
              )
              callback()
            }
          })
            ...
            if (err) {
              ...
              users.forEach(function(user) {
                if (user.phone) {
                  console.error(
                    chalk.yellow(
                      `User ${user.email} has invalid phone number ${user.phone}.`
                    )
                  )
                } else if (user.isVolunteer) {
                  console.error(
                    chalk.yellow(
                      `Volunteer ${user.email} has not provided a phone number.`
                    )
                  )
                }
              })
